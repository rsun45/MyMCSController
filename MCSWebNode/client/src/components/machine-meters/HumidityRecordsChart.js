...
const HumidityRecordsChart = () => {
  // ✅ 初始数据（时间戳格式）
  const [data, setData] = useState(() => [
    { time: new Date('2024-01-01 00:00:00').getTime(), value: 7 },
    { time: new Date('2024-01-01 00:00:03').getTime(), value: 6 },
  ]);

  // ✅ 生成新数据（基于最新的 data 状态）
  const generateNewData = (lastTime) => {
    const newTime = lastTime + 3000; // 递增 3 秒
    const newValue = Math.random() * 15 + 5;
...
export default HumidityRecordsChart;